...
def get_items(start, page_length, price_list, pos_profile, item_group="", search_term=""):
	...
	items_data = frappe.db.sql("""
		...
		LIMIT
			{start}, {page_length}"""
		.format(
			permission_conditions=get_match_cond("Item", "item"),
			start=start,
...
def get_past_order_list(search_term, status, limit=20):
	...
	if search_term and status:
		invoices_by_customer = frappe.db.get_list('POS Invoice', filters={
			'customer': ['like', '%{}%'.format(search_term)],
			'status': status
		}, fields=fields)
		invoices_by_name = frappe.db.get_list('POS Invoice', filters={
		...
		invoice_list = frappe.db.get_list('POS Invoice', filters={
			'status': status
		}, fields=fields)
