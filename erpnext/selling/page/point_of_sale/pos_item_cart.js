			...
			`<div class="cart-container">
				<div class="abs-cart-container">
					...
					<div class="cart-header">
						<div class="name-header">${__("Item", null, "Product")}</div>
						<div class="qty-header">${__("Qty")}</div>
						<div class="rate-amount-header">${__("Amount", null, "Short")}</div>
					</div>
			...
			df: {
				...
				onchange: function() {
					if (flt(this.value) != 0) {
						...
						me.hide_discount_control(discount);
					} else {
		...
		if (!discount) {
			...
			this.$add_discount_elem.html(
				`<div class="edit-discount-btn">
					${this.get_discount_icon()} ${__("Additional {0}% discount applied", [String(discount).bold()])}
				</div>`
			);
		}
	}
				...
				transaction_container.append(
					`<div class="invoice-wrapper" data-invoice-name="${escape(invoice.name)}">
						<div class="invoice-name-date">
							<div class="invoice-name">${frappe.remove_abbr(invoice.name)}</div>
