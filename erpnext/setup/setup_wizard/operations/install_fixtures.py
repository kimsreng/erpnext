...
def install(country=None):
	...
	records += [{'doctype': 'Email Template', 'name': ("Leave Approval Notification"), 'response': response,\
		'subject': ("Leave Approval Notification"), 'owner': frappe.session.user}]

	records += [{'doctype': 'Email Template', 'name': ("Leave Status Notification"), 'response': response,\
		'subject': ("Leave Status Notification"), 'owner': frappe.session.user}]

	base_path = frappe.get_app_path("erpnext", "stock", "doctype")
	response = frappe.read_file(os.path.join(base_path, "delivery_trip/dispatch_notification_template.html"))

	records += [{'doctype': 'Email Template', 'name': ("Dispatch Notification"), 'response': response,\
		'subject': ("Your order is out for delivery!"), 'owner': frappe.session.user}]
