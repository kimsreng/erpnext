...
def boot_session(bootinfo):
	...
	if frappe.session['user']!='Guest':
		...
		bootinfo.sysdefaults.territory = frappe.get_single_value('Selling Settings',
			'territory')
		bootinfo.sysdefaults.customer_group = frappe.get_single_value('Selling Settings',
			'customer_group')
		bootinfo.sysdefaults.allow_stale = cint(frappe.get_single_value('Accounts Settings',
			'allow_stale'))
		bootinfo.sysdefaults.quotation_valid_till = cint(frappe.get_single_value('Selling Settings',
			'default_valid_till'))
		...
		party_account_types = frappe.db.sql(""" select name, ifnull(account_type, '') from `tabParty Type` where 1=1 {0}""".format(get_match_cond("Party Type")))
