...
def get_project_details():
	return frappe.db.sql(""" select name, project_name, status, company, customer, estimated_costing,
		expected_start_date, expected_end_date from tabProject where docstatus < 2 {permission_cond}
		""".format(permission_cond=get_match_cond_for_reports("Project")), as_dict=1)
...
def get_purchased_items_cost():
	pr_items = frappe.db.sql("""select project, sum(base_net_amount) as amount
		...
		and project in (select name from tabProject where 1=1 {permission_cond})
		and docstatus = 1 group by project""".format(permission_cond=get_match_cond_for_reports("Project")), as_dict=1)
...
def get_issued_items_cost():
	se_items = frappe.db.sql("""select se.project, sum(se_item.amount) as amount
		...
		and ifnull(se.project, '') != '' {permission_cond} group by se.project
		""".format(permission_cond=get_match_cond_for_reports("Stock Entry", "se")), as_dict=1)
...
def get_delivered_items_cost():
	dn_items = frappe.db.sql("""select dn.project, sum(dn_item.base_net_amount) as amount
		...
		where dn.name = dn_item.parent and dn.docstatus = 1 and ifnull(dn.project, '') != '' {permission_cond}
		group by dn.project""".format(permission_cond=get_match_cond_for_reports("Delivery Note", "dn")), as_dict=1)
	...
	si_items = frappe.db.sql("""select si.project, sum(si_item.base_net_amount) as amount
		...
		and si.is_pos = 1 and ifnull(si.project, '') != '' {permission_cond}
		group by si.project""".format(permission_cond=get_match_cond_for_reports("Sales Invoice", "si")), as_dict=1)
