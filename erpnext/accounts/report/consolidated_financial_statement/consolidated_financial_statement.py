...
def execute(filters=None):
	...
	if filters.get('report') == "Balance Sheet":
		...
		if cint(frappe.get_single_value('Accounts Settings', 'use_custom_cash_flow')):
...
def get_accounts(root_type, filters):
	return frappe.db.sql(""" select name, is_group, company,
		...
		""".format(permission_cond=get_match_cond_for_reports("Account")) , (filters.get('company'), root_type), as_dict=1)
...
def set_gl_entries_by_account(from_date, to_date, root_lft, root_rgt, filters, gl_entries_by_account,
	...
	for d in companies:
		gl_entries = frappe.db.sql("""select gl.posting_date, gl.account, gl.debit, gl.credit, gl.is_opening, gl.company,
			...
			""".format(additional_conditions=additional_conditions, permission_cond=get_match_cond_for_reports("GL Entry", "gl")),
			{
				"from_date": from_date,
				"to_date": to_date,
				"lft": root_lft,
