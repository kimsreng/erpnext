...
def execute():
	...
	if frappe.db.exists("DocType", "Data Migration Plan"):
		data_migration_plans = frappe.get_all_with_user_permissions("Data Migration Plan", filters={"module": 'Hub Node'})
		for plan in data_migration_plans:
			...
			docs = frappe.get_all_with_user_permissions("Data Migration Run", filters={"data_migration_plan": plan.name})
			for doc in docs:
