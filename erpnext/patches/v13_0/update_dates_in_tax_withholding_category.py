...
def execute():
	...
	if frappe.db.has_column('Tax Withholding Rate', 'fiscal_year'):
		tds_category_rates = frappe.get_all_with_user_permissions('Tax Withholding Rate', fields=['name', 'fiscal_year'])

		fiscal_year_map = {}
		fiscal_year_details = frappe.get_all_with_user_permissions('Fiscal Year', fields=['name', 'year_start_date', 'year_end_date'])
